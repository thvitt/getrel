...
def download_releases(project: str):
    ...
    stable_releases = sorted(
        [rel for rel in releases if not rel["draft"] and not rel["prerelease"]],
        key=lambda r: r["created_at"],
        reverse=True,
    )
    choices = [
        questionary.Choice(
            title=f'{r["name"]} ({r["tag_name"]})',
            value=r,
            disabled="no asset" if not r.get("assets", []) else None,
        )
        for r in releases
    ]
    latest = None
    ...
    if stable_releases:
        ...
        choices.insert(
            0,
            questionary.Choice(
                title=f'Latest: {latest["name"]} (latest["tag_name"])',
                value="latest",
    ...
    release = questionary.select(
        "Which release do you want to download?",
        choices=choices,
        use_shortcuts=True,
        use_arrow_keys=True,
    ).ask()
    ...
    if len(release["assets"]) > 1:
        choices = [
            questionary.Choice(title=f'{a["label"] or ""} ({a["name"]})', value=a)
            for a in release["assets"]
        ]
        asset = questionary.select(
            "Select the file to download", choices=choices, use_shortcuts=True
        ).ask()
    ...
    with Progress(
        ...
        response = session.get(
            asset["url"], headers={"Accept": "application/octet-stream"}, stream=True
        )
